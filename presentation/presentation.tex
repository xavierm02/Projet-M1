...
\begin{frame}{Données d'entrée du problème}
\begin{figure}[H]
	\centering
    	\begin{tikzpicture}
    	    \node[] (x) at (-1,0) {};
...
\begin{figure}[H]
	\centering
    	\begin{tikzpicture}
            ...
            \draw [->,>=latex] (5) edge[in=100,out=80,loop] node[above] {$a,b$} (5); 
        \end{tikzpicture}
 \caption{Système $A$, $\Sigma_{no} = \{f\}$}
\end{figure}
\pause
Diagnosticabilité : Est qu'il est toujours possible de s'apercevoir qu'une faute s'est produite en observant $\Sigma_o$ ?
\end{frame}
...
\centering
On dit que $L$ est diagnosticable dans le système $A$ si :
$$\begin{array}{l}
