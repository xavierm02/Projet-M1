...
\forall l \in \Sigma, \forall u \in \Sigma^*, &\pi_{\Sigma'}(lu) &= &\left\{\begin{array}{ll}
l\pi_{\Sigma'}(u) &\text{ si } l \in \Sigma'\\
\pi_{\Sigma'}(u) &\text{ si } l \not\in \Sigma'
\end{array}\right.
\end{array}$$
...
L'id\'ee de ce th\'eor\`eme est que la non inteférence, c'est à dire le fait de ne pas rendre visible les actions d'un alphabet $\Sigma_h \subseteq \Sigma$ peut se récrire comme l'opacité du langage $\Sigma^*\Sigma_h\Sigma^*$.
