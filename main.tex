...
On peut prouver que $\mathcal L(\det(A))=\mathcal L(A)$ et que $\det(A)$ est un automate d\'eterministe (et fini si $A$ l'est).

\begin{mydef}[Compl\'ementaire d'un automate]
Le compl\'ementaire d'un automate $A = (Q, \Sigma, \Delta, q_0, F)$ sans $\varepsilon$-transitions est l'automate $$\overline{A} = (Q, \Sigma, \Delta, q_0, Q\setminus F)$$
\end{mydef}
...
\begin{mydef}
  Soit $A$ un automate que nous cherchons \`a diagnostiquer et $L$ un automate de mod\`ele de fautes que nous voulons diagnostiquer. Alors nous d\'efinissons $D : \Sigma_o^* \to \{\mbox{Oui,Non,Peut-\^etre}\}$ comme un diagnostiquer sur l'automate $A$ et le mod\`ele de faute $L$, si et seulement si, soit $w \in \Sigma_o^*$ qui est accept\'e par l'automate $A$,
  \begin{itemize}
