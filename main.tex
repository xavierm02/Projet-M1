...
\begin{enumerate}
  ...
  \item Construire $C=\operatorname{cl}_\varepsilon(B[\varepsilon/no])$ (Nous d\'efinission $B[\varepsilon/no]$ comme l'automate dans lequel nous avons remplac\'e toutes les lettres non-observables dans $\Sigma_{no}$ par $\varepsilon$)
...
%TODO intuition et/ou exemple : comment fonctionne le diagnostiqueur
...
Jeremy Bryans, Maciej Koutny, Laurent Mazar{\'{e}}, Peter Y. A. Ryan aont propos\'e dans leur article ~\cite{BryansKMR08}

Soit $LTS=(S,L,\Delta,S_0)$ est un syst\`eme de transition d'\'etats \'etiquett\'e, et $\L_{obs}$ est un ensemble d'\étiquettes $observables$. Remarque nous notons $L_{obs}^{\varepsilon} = L_{obs} \cup \{\varepsilon\}$
...
%TODO : Quelques lignes en plus pour l'intuition des preuves
