...
Ce syst\`eme sans la fl\`eche $b$  de $O$ vers $S$ est T-diagnosticable. Si on ajoute cette fl\`eche $b$, il n'est plus T-diagnosticable. En effet, consid\'erons les deux ex\'ecutions suivantes:
...
\paragraph{}
\noindent On prend $n := n_0$, $\forall u \in L_{F}(S)$, $\forall v \in \Sigma^*$, on prend $v' := v$. 
\begin{equation}
