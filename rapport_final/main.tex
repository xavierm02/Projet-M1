...
\paragraph{}
\noindent Soit $A$ le système que l'on cherche à diagnostiquer et $F$ l'automate de modèle de faute. 
$A$ est T-Diag par rapport à $F$ si et seulement si :
\newline
\newline
$ \forall u \in L_F.\ \exists n \in \mathbb{N}.\ \forall v \in \Sigma^*.\ \exists v' \in \Sigma^*. \\(uv \in L_A \wedge ||v||_{\Sigma_o}\geq n) \Rightarrow (v'\leq v \wedge u.Pref(v') \subseteq L_F \wedge [uv'] \cap L_A \subseteq L_F) $
\paragraph{}
